...
1. **Run the full evaluation** (ROADMAP AI-19; commands in `tests/eval/README.md`), then fill
   Table V ("Quality by arm"). Every value there is a red `\tbd{--}` placeholder. Take the values from
   `tests/eval/report/<name>/eredmenyek_tabla.csv`:

   | Table V column | CSV column |
   | --- | --- |
   ...
   Then add one or two sentences of findings under Table V, with p-values and effect sizes from
...
| Paper | Source |
| --- | --- |
| Table II (dataset), Fig. 3 | `tests/eval/dataset/manifest*.yaml` (39 documents, 326 reference questions) |
...
| Table IV (latency) | `tests/Test_Result_20260327_125252/e2e_report_20260327_125252.md` |
