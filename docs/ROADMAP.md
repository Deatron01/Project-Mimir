...
| ID | Task | Pri | Pts | Acceptance criteria |
| --- | --- | --- | --- | --- |
| [BIF-01](https://github.com/Deatron01/Project-Mimir/issues/25) | Session-scoped collections: one Qdrant collection (or payload filter) per `session_id`; delete on completion; remove global `clear_database()` | P0 | 3 | Two concurrent users never see each other's chunks (integration test) |
...
| [FE-05](https://github.com/Deatron01/Project-Mimir/issues/54) | Generation wizard: upload → configure (count, types, difficulty, language) → progress → edit → export | P1 | 8 | Replaces free-text chat as the main path |
...
| --- | --- | --- | --- | --- | --- |
| S1 | 28 Sep – 11 Oct | Stop the bleeding | PLT-01, PLT-02, PLT-04, PLT-05, BIF-01, BIF-02, RUN-01, HEI-01, FE-01, FE-03 | R1, R2 | CI green on every PR; no shared vector store; `make up` works on both GPU types |
| S2 | 12 – 25 Oct | Identity and privacy | GW-01, GW-02, GW-03, GW-06, FE-04, FE-10, SKA-01, SKA-02, WEL-04, FRG-01, FRG-02, FRG-04, FRG-05 | R3, R6 | **M1 multi-user alpha:** real login, zero files left after a job, no OOM with 5 concurrent jobs |
| S3 | 26 Oct – 8 Nov | AI core v1 | BIF-03, BIF-04, BIF-05, BIF-07, HEI-02, RUN-02, RUN-03, RUN-04, SHR-01, SHR-02, FE-02 | R4, R7 | Requested question count and types always honoured; naive-RAG baseline scores recorded |
| S4 | 9 – 22 Nov | Agentic generation | BIF-06, HEI-03, FRG-03, FRG-06, FE-05, FE-07, FE-08 | R5 | **M2 beta:** new pipeline beats baseline on eval set; wizard UI in HU/EN with themes |
| S5 | 23 Nov – 6 Dec | Editor and exports | FE-06, SKA-03, SKA-04, WEL-01, WEL-02, WEL-03, HEI-04, HEI-05, BIF-08 | — | Teacher can edit, regenerate one question, export PDF + Moodle; Playwright E2E covers it |
...
- Two concurrent sessions never retrieve each other's chunks (BIF-01).
...
| [AI-07](https://github.com/Deatron01/Project-Mimir/issues/67) | Graph store (Postgres tables, session-scoped) + neighbour queries | P1 | 3 | S4 |
...
| Extracted text, chunks, embeddings, concept graph | Redis / Qdrant / Postgres rows tagged `session_id`, tmpfs-backed volumes | Job duration, hard TTL 60 min | Delete on job end + TTL (Redis `EXPIRE`) + janitor sweep every 5 min |
...
| [GDPR-02](https://github.com/Deatron01/Project-Mimir/issues/74) | Session model: `session_id` on every artefact; per-session AES-GCM key held in Redis with TTL; spills to disk only as ciphertext on tmpfs | P0 | S2 |
...
| Vector store | Qdrant `:memory:`, one global collection | Qdrant server, per-session collections or payload-filtered shards, tmpfs storage | Qdrant replicas; short-lived data keeps it small |
...
| Main flow | Chat box that calls 4–5 services in sequence from the browser | 4-step wizard: Upload → Configure (count, types, difficulty, exam language, Fast/Thorough) → Progress (real stages via SSE, cancel) → Review |
...
| [FE-UX-01](https://github.com/Deatron01/Project-Mimir/issues/90) | App shell, routing split (public vs app), user menu with settings | P1 | 3 | S3 |
