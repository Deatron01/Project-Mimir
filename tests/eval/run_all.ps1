...
param(
    [string[]]$Arms = @("E0", "E1", "E2", "E2f", "E2h", "E3", "E4", "E4b", "E5a", "E5b", "E5c"),
...
if ($Arms -contains "E4" -or $Arms -contains "E4b") {
    python -m mimir_eval check "configs\$($configs['E4'])"
    if ($LASTEXITCODE -ne 0) { Write-Host "GenAI server not reachable: remove E4/E4b from -Arms or fix the key."; exit 1 }
}
